...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    const { data: tierData, error: tierError } = await supabase
      .from('events')
      .select('tier_1_enabled, tier_1_name, tier_1_price, tier_2_enabled, tier_2_name, tier_2_price, price_inr')
      .eq('id', eventIdStr)
      .single()
  ...
  if (req.method === 'POST') {
    ...
    // Validate inputs
    if (typeof tier1Enabled !== 'boolean' || typeof tier2Enabled !== 'boolean') {
